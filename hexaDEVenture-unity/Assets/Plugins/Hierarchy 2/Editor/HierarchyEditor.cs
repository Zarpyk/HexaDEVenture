...
namespace Hierarchy2 {
    ...
    public sealed class HierarchyEditor {
        ...
        private sealed class HierarchyWindow {
            ...
            public readonly int instanceID = int.MinValue;
        ...
        internal static class Styles {
            ...
            internal static GUIStyle PR_DisabledLabel = new("PR DisabledLabel");

            internal static GUIStyle PR_PrefabLabel = new("PR PrefabLabel");

            internal static GUIStyle PR_DisabledPrefabLabel = new("PR DisabledPrefabLabel");

            internal static GUIStyle PR_BrokenPrefabLabel = new("PR BrokenPrefabLabel");

            internal static GUIStyle PR_DisabledBrokenPrefabLabel = new("PR DisabledBrokenPrefabLabel");

            internal static GUIStyle Tag = new() {
            ...
            internal static GUIStyle Layer = new() {
                padding = new RectOffset(3, 4, 0, 0),
                alignment = TextAnchor.MiddleCenter,
                fontStyle = FontStyle.Italic,
                fontSize = 8,
            ...
            internal static GUIStyle Header = new(TreeBoldLabel) {
                richText = true,
        ...
        internal sealed class MenuCommand {
            ...
            private static bool ValidateSetNotEditableObject() {
            ...
            private static bool ValidateSetEditableObject() {
                return Selection.gameObjects.Length > 0;
            }
            ...
            private static bool ValidateQuickSiblingUp() {
            ...
            private static bool ValidateQuickSiblingDown() {
                return Selection.activeTransform != null;
            }

            [MenuItem("Tools/Hierarchy 2/Separator", priority = 0),
             MenuItem("GameObject/Separator", priority = 0)]
            private static void CreateHeaderInstance(UnityEditor.MenuCommand command) {
                GameObject gameObject = new(string.Format("{0}Separator", instance.settings.separatorStartWith));
        ...
        private static List<Type> DisplayOnHierarchyScriptType = new();
        private static Dictionary<string, Type> dicDisplayOnHierarchyScriptType = new();

        private static Type SceneHierarchyWindow;
        private static Type SceneHierarchy;
        private static Type GameObjectTreeViewGUI;

        private static FieldInfo m_SceneHierarchy;
        private static FieldInfo m_TreeView;
        private static PropertyInfo gui;
        private static FieldInfo k_IconWidth;
        ...
        private static Func<GameObject, Rect, bool, bool> IconSelectorShowAtPositionDelegate;
        ...
        private readonly Dictionary<int, Object> selectedComponents = new();
        private readonly Dictionary<string, string> dicComponents = new(StringComparer.Ordinal);
        ...
        public static HierarchyEditor Instance {
            get {
                if (instance == null) instance = new HierarchyEditor();
                return instance;
            }
        ...
        public static IEnumerable GetAllSceneHierarchyWindows() {
            return GetAllSceneHierarchyWindowsDelegate();
        }

        public static void DisplayObjectContextMenu(Rect rect, Object unityObject, int value) {
        ...
        public static bool IconSelectorShowAtPosition(GameObject gameObject, Rect rect, bool value) {
            return IconSelectorShowAtPositionDelegate(gameObject, rect, value);
        }
        ...
        private static void InternalReflection() {
            Type[] arrayInteralEditorType = typeof(Editor).Assembly.GetTypes();
            ...
            FieldInfo refreshHierarchy =
                typeof(EditorApplication).GetField(nameof(refreshHierarchy),
                                                   BindingFlags.Static | BindingFlags.NonPublic);
            MethodInfo OnRepaintHierarchyWindow =
                typeof(HierarchyEditor).GetMethod(nameof(OnRepaintHierarchyWindow),
            ...
            FieldInfo windowsReordered =
                typeof(EditorApplication).GetField(nameof(windowsReordered),
                                                   BindingFlags.Static | BindingFlags.NonPublic);
            MethodInfo OnWindowsReordered =
                typeof(HierarchyEditor).GetMethod(nameof(OnWindowsReordered),
                                                  BindingFlags.NonPublic | BindingFlags.Static);
            ...
            FieldInfo s_LastInteractedHierarchy =
                SceneHierarchyWindow.GetField(nameof(s_LastInteractedHierarchy),
                                              BindingFlags.NonPublic | BindingFlags.Static);

            MethodInfo lastInteractedHierarchyWindow = SceneHierarchyWindow
                                                      .GetProperty(nameof(lastInteractedHierarchyWindow),
            ...
            MethodInfo GetAllSceneHierarchyWindows =
                SceneHierarchyWindow.GetMethod(nameof(GetAllSceneHierarchyWindows),
                                               BindingFlags.Static | BindingFlags.Public);
            ...
            {
                m_SceneHierarchy =
                    SceneHierarchyWindow.GetField(nameof(m_SceneHierarchy),
            ...
            Type IconSelector = typeof(EditorWindow).Assembly.GetTypes().Single(type =>
                                                                                    ...
                                                                                    type.Name ==
                                                                                    nameof(IconSelector));
            MethodInfo ShowAtPosition = IconSelector.GetMethods(BindingFlags.Static | BindingFlags.NonPublic)
                                                    .Single(method => method.Name == nameof(ShowAtPosition) &&
        ...
        private static void OnRepaintHierarchyWindow() {
            OnRepaintHierarchyWindowCallback?.Invoke();
        }

        private static void OnWindowsReordered() {
        ...
        private void EditorAwake() {
            ...
            if (settings.activeHierarchy) Invoke();
            else Dispose();
        ...
        private void OnSettingsChanged(string param) {
            switch (param) {
                case nameof(settings.components):
                    dicComponents.Clear();
                    foreach (string componentType in settings.components) {
                        if (!dicComponents.ContainsKey(componentType)) dicComponents.Add(componentType, componentType);
                    }
        ...
        private void DelayCall() {
            ...
            if (hierarchyChangedRequireUpdating) hierarchyChangedRequireUpdating = false;
        }

        private void OnModifierKeysChanged() { }

        [DidReloadScripts]
        private static void OnEditorCompiled() { }
        ...
        private void OnPrefabUpdated(GameObject prefab) { }
        ...
        private void HierarchyOnGUI(int selectionID, Rect selectionRect) {
            ...
            if (!settings.activeHierarchy) return;

            if (currentEvent.control && currentEvent.keyCode == KeyCode.D) return;

            if (currentEvent.type == EventType.Layout) {
                if (prefabStageChanged) prefabStageChanged = false;
            ...
            if (rowItem.isFirstRow) //Instance always null
            {
                ...
                if (deepestRow > previousRowIndex) deepestRow = previousRowIndex;
            ...
            if (rowItem.isNull) {
                if (!IsMultiScene) {
                    ...
                    if (!rowItem.isFirstRow && sceneIndex < SceneManager.sceneCount - 1) sceneIndex++;
                ...
                if (previousRowIndex > deepestRow) deepestRow = previousRowIndex;
                return;
            }
            ...
            if (settings.useInstantBackground) CustomRowBackground();

            if (settings.displayTreeView && !rowItem.isRootObject) DisplayTreeView();

            if (settings.displayCustomObjectIcon) DisplayCustomObjectIcon(null);
            ...
            FINISH:
            if (settings.displayGrid) DisplayGrid();
        ...
        private GUIStyle TreeStyleFromFont(FontStyle fontStyle) {
            GUIStyle style;
            switch (fontStyle) {
                case FontStyle.Bold:
        ...
        private void CustomRowBackground() {
            ...
            HierarchySettings.InstantBackgroundColor instantBackgroundColor = new();
            bool contain = false;
        ...
        private void ElementAsSeparator() {
            ...
            GUIContent content = new(rowItem.name.Remove(0, settings.separatorStartWith.Length));
        ...
        private void ElementEvent(RowItem element) {
            ...
            if (currentEvent.type == EventType.KeyUp &&
                ...
                SelectionsRenamePopup window = SelectionsRenamePopup.ShowPopup();
                currentEvent.Use();
                return;
            }
        ...
        private void StaticIcon(RowItem element) {
        ...
        private void ApplyStaticTargetAndChild(Transform target, bool value) {
        ...
        private void DisplayCustomObjectIcon(Texture icon) {
            ...
            if (currentEvent.type == EventType.Repaint) {
                ...
                if (icon == null) {
                    ...
                    if (icon.name == "GameObject Icon" || icon.name == "d_GameObject Icon" ||
                        ...
                        icon.name == "d_PrefabModel Icon") return;
                }
                    ...
                    if (comType != null) {
                        ...
                        switch (settings.componentDisplayMode) {
                            ...
                            case HierarchySettings.ComponentDisplayMode.Specified:
                                if (!dicComponents.ContainsKey(comType.Name)) continue;
                                break;
                        ...
                        else rect = RectFromRight(rowItem.rect, settings.componentSize, ref widthUse.right);
                        ...
                        if (hasMaterial && i == length - rendererComponent.sharedMaterials.Length &&
                            ...
                            for (int m = 0; m < rendererComponent.sharedMaterials.Length; ++m) {
                                Material sharedMaterial = rendererComponent.sharedMaterials[m];
                        ...
                        else widthUse.right += settings.componentSpacing;
        ...
        private void ComponentIcon(Object component, Type componentType, Rect rect, bool isMaterial = false) {
            ...
            if (rect.Contains(currentEvent.mousePosition)) {
                if (currentEvent.type == EventType.MouseDown) {
                    ...
                    if (currentEvent.button == 1) {
                        if (currentEvent.control) {
                            GenericMenu componentGenericMenu = new();

                            componentGenericMenu.AddItem(new GUIContent("Remove All Component"), false, () => {
                                if (!selectedComponents.ContainsKey(comHash))
                                    selectedComponents.Add(comHash, component);

                                foreach (KeyValuePair<int, Object> selectedComponent in selectedComponents.ToList()) {
        ...
        private void BottomRightArea(Rect rect) {
        ...
        private void Background(Rect rect) {
            // rect.y += 16;
            // rect.xMin = 0;
        ...
        private void ApplyTagTargetAndChild(Transform target, string tag) {
                    ...
                    menuLayers.AddItem(new GUIContent(layer),
                                       LayerMask.NameToLayer(layer) == gameObject.layer ? true : false, () => {
                                           if (settings.applyLayerTargetAndChild) {
                                               ApplyLayerTargetAndChild(gameObject.transform,
        ...
        private void ApplyLayerTargetAndChild(Transform target, int layer) {
        ...
        private void DisplayRowBackground(bool nextRow = true) {
            if (currentEvent.type != EventType.Repaint) return;
        ...
        private void DisplayTreeView() {
            ...
            else GUI.DrawTexture(rect, resources.GetIcon("icon_branch_T"), ScaleMode.ScaleToFit);
        ...
        private void RenameSceneInHierarchy() {
            ...
            float leftTitleWidthUsed = 48f;
#if UNITY_2019_1_OR_NEWER
            leftTitleWidthUsed += 24f;
#endif

            if (!currentScene.isLoaded) name = string.Format("{0} (not loaded", name);
            ...
            if (currentEvent.type == EventType.KeyDown &&
                currentEvent.keyCode == KeyCode.F2 &&
                rowItem.rect.Contains(currentEvent.mousePosition)) SceneRenamePopup.ShowPopup(currentScene);
        }

        private void CollapseAll() { }

        private void DirtyScene(Scene scene) {
            if (EditorApplication.isPlaying) return;

            EditorSceneManager.MarkSceneDirty(scene);
        }

        private bool IsFirstElement(Rect rect) {
            return previousRowIndex > rect.y / rect.height;
        }

        private bool IsFirstRow(Rect rect) {
            return rect.y / rect.height == 0;
        }

        private int GetRowIndex(Rect rect) {
            return (int)(rect.y / rect.height);
        }

        private bool InSelection(int ID) {
            return Selection.Contains(ID) ? true : false;
        }

        private bool IsElementDirty(int ID) {
            return EditorUtility.IsDirty(ID);
        }
        ...
        private Rect RectFromRight(Rect rect, float width, ref float usedWidth) {
            usedWidth += width;
            rect.x = rect.x + rect.width - usedWidth;
            rect.width = width;
            return rect;
        ...
        private Rect RectFromRight(Rect rect, Vector2 offset, float width, ref float usedWidth) {
            usedWidth += width;
        ...
        private Rect RectFromLeft(Rect rect, float width, float usedWidth, bool usexmin = true) {
            if (usexmin) rect.xMin = 0;
            rect.x += usedWidth;
            rect.width = width;
            usedWidth += width;
            return rect;
        ...
        private Rect RectFromLeft(Rect rect, float width, ref float usedWidth, bool usexmin = true) {
            if (usexmin) rect.xMin = 0;
            rect.x += usedWidth;
            rect.width = width;
            usedWidth += width;
        ...
        private Rect RectFromLeft(Rect rect, Vector2 offset, float width, ref float usedWidth, bool usexmin = true) {
            if (usexmin) rect.xMin = 0;
            rect.position += offset;
            rect.x += usedWidth;
            rect.width = width;
        ...
        private void GUISeparator(Rect rect, Color color) {
            Color guiColor = GUI.color;
            GUI.color = color;
            GUI.DrawTexture(rect, Resources.PixelWhite, ScaleMode.StretchToFill);
            GUI.color = guiColor;
